...
{
    public class AppointmentController : BeautyCenterManagerController
    {
        #region Fields
        ...
        public async Task<IActionResult> Index(AppointmentStatus? currentStatus = null, string fromDate = "", string toDate = "")
        {
            ...
            return View(new BeautyCenterAppointmentListViewModel());
        }
        ...
        public async Task<IActionResult> LoadTable([FromBody]DataTablesParameters param)
        {
            try
            {
                ...
                var results = await _appointmentService.GetDataTableAsync(CurrentBeautyCenter.Id, param, filtersModel, Lng, CurrentBeautyCenter.ServiceSupplyIds);

                var turns = await Task.WhenAll(results.Items.Select(async (x) => new BeautyCenterAppointmentListViewModel
                {
                    Id = x.Id,
                    Status = x.Status,
                    ReservationChannel = x.ReservationChannel,
                    Date = x.Date,
                    Employee = x.ServiceSupplier,
                    Person = x.Person,
                ...
                return new JsonResult(new DataTablesResult<BeautyCenterAppointmentListViewModel>
                {
                    Draw = param.Draw,
                    Data = turns.ToList(),
            ...
            try
            {
                var appointment =
                    ...
                    .FirstOrDefault(x => x.Id == id &&
                                         x.ServiceSupply.ShiftCenterId == CurrentBeautyCenter.Id &&
                                         x.Status != status &&
                                         !x.IsDeleted);
                if (appointment == null)
                {
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                var appointment = _appointmentService.GetAllForPolyClinic(CurrentBeautyCenter.Id).FirstOrDefault(x => x.Id == id);
                if (appointment == null)
                {
                    throw new EntityNotFoundException();
                }
        ...
        public IActionResult HardDelete(int id)
        {
            try
            {
                var appointment = _dbContext.Appointments.FirstOrDefault(x => x.Id == id && x.ServiceSupply.ShiftCenterId == CurrentBeautyCenter.Id && x.IsDeleted);
