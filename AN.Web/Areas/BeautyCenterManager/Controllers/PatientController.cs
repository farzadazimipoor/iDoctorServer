...
{
    public class PatientController : BeautyCenterManagerController
    {
        ...
        public async Task<IActionResult> LoadTable([FromBody]DataTablesParameters param)
        {
            try
            {
                ...
                var results = await _patientService.GetDataTableAsync(CurrentBeautyCenter.Id, param, filtersModel, Lng);
                ...
                var persons = await Task.WhenAll(results.Items.Select(async (x) => new PatientListViewModel
                {
                    ...
                    Doctor = x.Doctor,
                    Avatar = x.Avatar,
                    AvatarHtml = await this.RenderViewToStringAsync("_PatientItemAvatar", x.Avatar),
                    ActionsHtml = await this.RenderViewToStringAsync("_PatientItemActions", (x.Id, x.PersonId, x.ServiceSupplyId))
                }).ToList());
        ...
        [HttpGet]
        public async Task<IActionResult> Find(int appointmentId, string actionRequest = "pharmacy")
        {
            if (appointmentId <= 0) throw new ArgumentNullException(nameof(appointmentId));

            var appointment = await _appointmentService.GetAppointmentByIdAsync(appointmentId);

            if (appointment == null) throw new Exception("Appointment Not Found");

            if (!appointment.TreatmentHistories.Any())
            {
                if (actionRequest == "pharmacy")
                {
                    return RedirectToAction("Create", "TreatmentHistory", new { area = "polyClinicManager", appointmentId = appointment.Id });
                }
                else if (actionRequest == "sonar")
                {
                    return RedirectToAction("CreateSonarRequest", "TreatmentHistory", new { area = "polyClinicManager", appointmentId = appointment.Id });
                }
                else if (actionRequest == "lab")
                {
                    return RedirectToAction("CreateLabRequest", "TreatmentHistory", new { area = "polyClinicManager", appointmentId = appointment.Id });
                }
            }
            if (appointment.TreatmentHistories.Any())
            {
                var resultModel = new PersonInfoViewModel
                {
                    UniqueId = appointment.Person.UniqueId,
                    FirstName = appointment.Person.FirstName,
                    SecondName = appointment.Person.SecondName,
                    ThirdName = appointment.Person.ThirdName
                };

                return PartialView("NotifyHistoryCreated", resultModel);
            }

            var model = new FindPatientViewModel
            {
        ...
        [HttpPost]
        public async Task<IActionResult> FindByUniqueId(FindPatientViewModel model)
        {
            var patient = await _personService.FindByUniqueIdAsync(model.UniqueId);

            if (patient == null)
            {
                return RedirectToAction("CreateUpdate", new { appointmentId = model.AppointmentId });
            }
            else
            {
                return RedirectToAction("Create", "TreatmentHistory", new { area = "PolyClinicManager", appointmentId = model.AppointmentId, patientId = patient.Id });
            }
        }
        ...
        public async Task<IActionResult> FindPerson(FindPersonModel model)
        {
            ...
            {
                var createModel = new CreatePatientFromPersonViewModel
                {
                    DoctorsList = (await _serviceSupplyService.GetSelectListAsync(CurrentBeautyCenter.Id, CurrentBeautyCenter.ServiceSupplyIds)).ToList(),
        ...
        {
            var existBefore = await _patientService.FindForServiceSupplyAsync(model.ServiceSupplyId.Value, model.PersonInfo.Id) != null;

            if (existBefore) throw new AwroNoreException(Global.CustomerAlreadyExist);
        ...
        [HttpPost]
        public async Task<IActionResult> CreateUpdate(CreatePatientViewModel model)
        {
