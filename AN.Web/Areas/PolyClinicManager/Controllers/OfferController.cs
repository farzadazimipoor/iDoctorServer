...
{
    public class OfferController : PolyclinicManagerController
    {
        ...
        public OfferController(IWorkContext workContext, 
                               ...
                               IScheduleManager scheduleManager) : base(workContext)
        {
        ...
        public async Task<IActionResult> LoadTable([FromBody]DataTablesParameters param)
        {
            try
            {
                ...
                var results = await _offerRepository.GetDataAsync(param, centerId: CurrentPolyclinic.Id, serviceSupplyIds: CurrentPolyclinic.ServiceSupplyIds, isForAdmin: false);
        ...
        public async Task<IActionResult> CreateUpdate(int? id = null)
        {
            ...
            ViewBag.HealthServices = _healthServiceService.GetShiftCenterServicesListItems(CurrentPolyclinic.Id, Lng).Select(x => new SelectListItem
            {
                Text = x.Text,
                Value = x.Value
            }).ToList();
        ...
        public async Task<IActionResult> CreateUpdate(OfferCreateUpdateViewModel model)
        {
            try
            {
                if (!CurrentPolyclinic.ServiceSupplyIds.Contains(model.ServiceSupplyId)) throw new AwroNoreException("You don not have permission to add offer to this doctor");
