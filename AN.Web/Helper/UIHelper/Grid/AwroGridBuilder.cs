...
{
    public class AwroGridBuilder<TSource, TDestination> : IActionResult
    {
        private List<TSource> _data;
        private List<TDestination> _destData;
        private readonly IMapper _mapper;
        public AwroGridBuilder(List<TSource> data, IMapper mapper)
        {
            _data = data;
    ...
    public class AwroGridBuilder<TDestination> : IActionResult
    {

        private List<TDestination> _destData;
        private int _elementsCountPerPage;
        private int _pageNumber;
        public AwroGridBuilder(List<TDestination> data)
        {
            _destData = data;
        }
        public AwroGridBuilder(List<TDestination> data,int elementsCountPerPage, int pageNumber) 
        {
            _destData = data;
        ...
        public async Task<List<object>> ConvertDataAsync(HttpContext context) 
        {
            ...
            if (_destData != null&& service!=null)
            {
                ...
                foreach (var item in _destData)
                {
                    ...
                    var obj = AwroTypeBuilder.CreateNewObject(await expandoObject);
                    result.Add(obj);
                    rowIndex++;
                }        
            }
        ...
        private async Task<ExpandoObject> createColumns(ActionContext context,TDestination obj,int rowIndex)
        {
            ...
            foreach (var item in properties)
            {
                ...
                //{
                    ...
                    else if (item.GetCustomAttribute<AwroGridBooleanToString>() != null)
        ...
        private void formateDateTime(TDestination obj, dynamic result, PropertyInfo item)
        {
            ...
            {              
                if (item.GetCustomAttribute<AwroGridDateTime>() != null)
                {
                    var date = ((DateTime)item.GetValue(obj));
                    var formatDate = !string.IsNullOrEmpty(item.GetCustomAttribute<AwroGridDateTime>().Format) ? item.GetCustomAttribute<AwroGridDateTime>().Format : "yyyy/MM/dd";
                    value = ((DateTime)item.GetValue(obj)).ToString(formatDate);
                    if (item.GetCustomAttribute<AwroGridDateTime>().ShowTime)
                    {
        ...
        private void formateBoolean(TDestination obj, dynamic result, PropertyInfo item)
        {
            ...
            if (item.GetValue(obj) != null)
            {
                ...
                if (originValue)
                    value = Resources.AwroGrid.CheckBox_True;
                else
                    value = Resources.AwroGrid.CheckBox_False;
            }
            AddProperty(result, item.Name, value, typeof(string));
        }
    ...
    public static class AwroTypeBuilder 
    {
